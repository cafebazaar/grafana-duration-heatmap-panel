...
export default function heatmap(data, num_of_slices, elem, svg_id, elem_size, date_domain, bin_domain, frq_domain, all_buckets) {
    ...
    var x_scale = d3.time.scale().range([0, rect_size*num_of_slices]).domain([date_domain.min, date_domain.max]);
    var y_scale = d3.scale.linear().range([rect_size*vertical_domain_length, 0]).domain([bin_domain.min, bin_domain.max]);
